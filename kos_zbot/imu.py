...
class BNO055Manager:
            ...
            try:
                ...
                if loop_time > self.target_period:
                    ...
                    if self.consecutive_overruns >= 3:
                        logging.warning(
                            f"IMU timing overrun: {loop_time*1000:.2f}ms (target: {self.target_period*1000:.2f}ms)"
                        )
                else:
                    self.consecutive_overruns = 0
    ...
    def get_timing_stats(self):
        ...
        stats = {
            ...
            "average_read_times": {
                "sensors": (
                    sum(self.timing_stats["accel"]) / len(self.timing_stats["accel"])
                    if self.timing_stats["accel"]
                    else 0
                ),
            },
        }
